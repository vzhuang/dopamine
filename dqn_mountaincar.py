...
DQNAgent.observation_shape = %gym_lib.MOUNTAINCAR_OBSERVATION_SHAPE
DQNAgent.observation_dtype = %gym_lib.MOUNTAINCAR_OBSERVATION_DTYPE
DQNAgent.stack_size = %gym_lib.MOUNTAINCAR_STACK_SIZE
DQNAgent.network = @gym_lib.MountainCarDQNNetwork
DQNAgent.gamma = 0.99
...
for run in range(10):
    ...
    DQN_PATH = os.path.join(BASE_PATH, 'mc_levy1.1/run_' + str(run))
    ...
    data = colab_utils.read_experiment(DQN_PATH, verbose=True,
                                       summary_keys=['train_episode_returns', 'train_episode_lengths'])
    data['agent'] = 'DQN'
    data['run'] = run
